...
def intentar_guardar_nota(from_number):
    sesion = obtener_sesion(from_number)
    titulo = sesion.get("titulo")
    cuerpo = sesion.get("cuerpo")
    categorias = sesion.get("categorias")
    autor_id = sesion.get("autor_id")

    if titulo and cuerpo and categorias and autor_id and not sesion.get("nota_id"):
