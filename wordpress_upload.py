...
def subir_imagen_remota_a_wordpress(url_remota, titulo_original, mime_type="image/jpeg"):
    ...
    try:
        ...
        if "api.twilio.com" in url_remota:
            ...
            response_img = requests.get(url_remota, auth=(twilio_sid, twilio_token))
        else:
            response_img = requests.get(url_remota)
        ...
        response = requests.post(wordpress_url, headers=headers, data=image_data.getvalue())
        if response.status_code in [200, 201]:
            data = response.json()
